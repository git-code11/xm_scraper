...
class XmMovieMapSpider(SitemapSpider):
	...
	def parse_movie(self,response):
		...
		_des = response.selector.css("div.detail_page.watch_page div.description::text")
		item['description'] = _des.get().strip() if _des else "NA"
		...
		for line in _extra:
			...
			if(re.match("released",_tmp[0],re.I)):
				_q_year = re.match(r"(\d{4})-.*",_tmp[1])
				item['year'] = _q_year[1] if _q_year else "NA"
